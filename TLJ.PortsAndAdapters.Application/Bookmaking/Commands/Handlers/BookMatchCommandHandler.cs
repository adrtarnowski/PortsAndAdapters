...
using TLJ.PortsAndAdapters.Core.Domain.Rules;
    ...
    public class BookMatchCommandHandler : ICommandHandler<BookMatchCommand>
    {
        ...
        public async Task HandleAsync(BookMatchCommand command)
        {
            var currency = command.Currency ??
                throw new BrokenBusinessRuleException(
                    new RequiredValueException(nameof(command.Value)));
            var bookType = command.BookType ??
                           throw new BrokenBusinessRuleException(
                               new RequiredValueException(nameof(command.BookType)));
